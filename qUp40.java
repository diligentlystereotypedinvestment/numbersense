...
class qUp40
{
			...
			ArrayList < String > answers, int i)
	{
		...
		{				// Inverses
			...
			{
				printmess =
					("(" + i + ") What is the mutliplicative reciprocal of " +
					 denominator + "?");
			...
			else
			{
				...
				printmess =
					("(" + i + ") What is the additive reciprocal of " +
					 denominator + "?");
		...
		{				// Sets
			...
			{			// union
				...
				union.addAll (setB);
				printmess =
					("(" + i + ") How many elements are in the union of " + setA +
					 " and " + setB + "?");
			...
			{			// intercept
				...
				printmess =
					("(" + i + ") How many elements are in the intercept of " +
					 setA + " and " + setB + "?");
			...
			{			// proper
				printmess =
					("(" + i + ") How many proper subsets are in " + setA + "?");
				answers.add (String.
						valueOf (1.0 * Math.pow (2, setA.size ()) - 1));
			}
			if (choose == 4)
			{			// improper
				printmess =
					("(" + i + ") How many improper subsets are in " + setA + "?");
			...
			{			// choose
				...
				{
					printmess =
						("(" + i + ") How many subsets with length " +
				...
				else if (doublechoose == 2)
				{
					printmess =
						("(" + i + ") How many subsets with length " +
						 chooselength + " are there in " + setA +
						 " + subsets of length " + (setA.size () - chooselength) +
						 " in " + setA + "?");
					answers.add (String.
							valueOf (2.0 *
								chose.choose (setA.size (),
			...
			{			// number of subsets
				printmess =
					("(" + i + ") How many subsets are there in " + setA + "?");
		...
		{				// Bases
			...
			if (choose == 1)
			{
				...
				printmess =
					("(" + i + ") What is " + ognumber + " in base " + base);
				answers.add (String.
						valueOf (baseconvert.newnum (ognumber, base)));
			}
			if (choose == 2)
			{
				ognumber = rand.nextInt (104) + 5;
			...
			if (choose == 3)
			{
				...
				printmess =
					("(" + i + ") What is $" + newnum + "_{" + choose2 +
					 "}$ in base " + secondchoose2);
				answers.add (String.
						valueOf (baseconvert.
							newnum (baseconvert.ognum (newnum, choose2),
								secondchoose2)));
			}
			questions.add (printmess);	// System.out.print(printmess);
			/*
		...
		{				// Solving Simple Equations// System.out.print(printmess);
		...
		{				// system of equations
			...
			else
			{
				...
				String printmess = "";
				printmess =
					("(" + i + ") $" + x1 + "x" + sign1 + y1 + "y = " + equal1 +
					 " , " + x2 + "x" + sign2 + y2 + "y = " + equal2 +
					 "$ What is the value of " + solve);
		...
		{				// I think this is more remainder problems
			int dividend = rand.nextInt (4) + 3;
			int base = rand.nextInt (12) + 2;
			int power = rand.nextInt (12) + 2;
			String printmess =
				("(" + i + ") What is the remainder of $" + base + "^{" + power +
				 "}\\div" + dividend + "?$");
			double exponentr = Math.pow (base, power);
			questions.add (printmess);
