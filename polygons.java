...
class polygons{
	...
	public void gen(ArrayList<String> questions, ArrayList<String> answers, int i){
		...
		if(sides == 0){//equilateral triangle
			if(problem == 0){//height
				questions.add("What is the height of a triangle with a side length of " + sideLength + "?");
				answers.add(sideLength/2 + "$\\sqrt{3}$");
			}	
			if(problem == 1){//area
				questions.add("What is the area of a triangle with a side length of " + sideLength + "?");
				answers.add("$" + simplify.getFraction((int)Math.pow(sideLength, 2) + "/" +  4, false) + "\\cdot\\sqrt{3}$");
			}
			if(problem == 2){//side length from area
				questions.add("What is the side length of a triangle with an area of $" + simplify.getFraction((int)Math.pow(sideLength, 2) + "/" +  4, false) + "\\cdot\\sqrt{3} $?");
				answers.add(String.valueOf(sideLength));
			}
		}
		if(sides == 1){//rectangle
			if(problem == 0){//diagonals
				questions.add("What is the diagonal length of a square  with an area of " + (int)Math.pow(sideLength, 2)+ "?");
				answers.add("$" + sideLength + "\\sqrt{2}$");
			}	
			if(problem == 1){//perimeter
				questions.add("What is the perimeter of a rectangle with a diagonal of " + sideLength + "$\\sqrt{2}$?");
			...
			if(problem == 2){//area ratios
				...
				questions.add("Rectangle ABCD is " + sideLength + "\" by " + side2 + "\" and rectangle A\'B\'C\'D\' is " + side3 + "\" by" + side4 + "\" What is the ratio of the area of A to B?");
	...
	public void gen2(ArrayList<String> questions, ArrayList<String> answers, int i){
		...
		if(sides == 2){//pentagons
			...
			if(problem == 1){//perimeter
				questions.add("What is the perimeter of a " + penta.getName() + "with a side length of " + sideLength);
				answers.add(String.valueOf(penta.getPerimeter()));
			}
			if(problem == 2){//interior angle measures
				questions.add("What is the interior angle measure of a " + penta.getName() + "?"); 
		...
		int tracker = 6;
		nagon.gen(sides, sideLength, tracker, 9, "nonagon", questions, answers);
		tracker++;
		nagon.gen(sides, sideLength, tracker, 10, "decagon", questions, answers);
		tracker++;
		nagon.gen(sides, sideLength, tracker, 11, "undecagon", questions, answers);
		tracker++;
		nagon.gen(sides, sideLength, tracker, 12, "dodecagon", questions, answers);
