...
public class romanNum
{
	...
	public void cleanup ()
	{				//code to fix four roman numerals in a row
		/*
		...
		while (!inARow (i) && i < romanChars.size () - 4)
		{
			...
			romanChars.add (i,
					LETTERS[getIndex (LETTERS, romanChars.get (i)) - 1]);
			i++;
		}
	}
	...
	public boolean inARow (int i)
	{				//code to detect when there are four in a row
		if (romanChars.get (i).equals (romanChars.get (i + 1))
				&& romanChars.get (i + 1).equals (romanChars.get (i + 2))
				&& romanChars.get (i + 2).equals (romanChars.get (i + 3)))
		{
			return false;
		}
		return true;
