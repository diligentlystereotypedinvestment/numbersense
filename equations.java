...
public class equations
{
	...
	public static void gen (int i, ArrayList < String > questions,
			ArrayList < String > answers)
	{
		...
		int[] coef =
		{ rand.nextInt (14) - 5, rand.nextInt (14) - 5, rand.nextInt (14) - 5 };
		...
		if (choose == 0)
		{				//two variable with one being substituted
			questions.add ("(" + i + ") What is the value of $" + variable1 +
					" $ if $" + coef[0] + variable1 + " = " + coef[1] +
					variable2 + " " + sign + " " + coef[2] + "$ and $" +
					variable2 + " = " + variableVal + "$?");
			answers.add (simplify.
					getFraction ((variableVal * coef[1] + coef[2]) + "/" +
						coef[0]));
		}
		if (choose == 1)
		{				//one variable
			questions.add ("(" + i + ") What is the value of $" + variable1 +
					"$ if $" + coef[0] + " = " + coef[1] + variable1 +
					" " + sign + " " + coef[2] + "$?");
			if (sign == '+')
			{
				answers.add (simplify.
						getFraction ((coef[0] + coef[2]) + "/" + coef[1]));
			}
			else
			{
				answers.add (simplify.
	...
	public static void genInequal (int i, ArrayList < String > questions,
			ArrayList < String > answers)
	{
		Random rand = new Random ();
		...
		String question =
			"(" + i + ") $" + coef[0] + variable + coef[1] + direction + coef[2] +
			", " + variable;
