...
public class polynomial
{
	...
	public void Gen (int i)
	{
		...
		{
			this.problem =
				"(" + i + ") What is the sum of roots of " + tempProblem + "?";
			answer =
				simplify.getFraction (String.valueOf (-1 * problem.getCoef (1)) +
						"/" + problem.getCoef (0));
		}
		if (choose == 1 && power == 2)
		{
			...
			{			// 0
				...
				int a =
					PrimeDivisors.factor (ac).get (rand.
							nextInt (PrimeDivisors.
								factor (ac).size ()));
				int c = ac / a;
			...
			this.problem =
				"(" + i + ") What is the discriminate of " + problem + "?";
		}
		else if (choose == 1 && power == 3)
		{
			this.problem =
				"(" + i + ") What is the sum of the roots of " + problem +
				" taken two at a time?";
			answer =
				simplify.getFraction (problem.getCoef (2) + "/" +
						problem.getCoef (0));
		}
		if (choose == 2)
		{
			this.problem =
				"(" + i + ") What is the product of the roots of " + problem;
			answer =
				simplify.getFraction (problem.getCoef (power) + "/" +
						(problem.getCoef (0) *
						 (int) Math.pow (-1, power)));
		}

	}

	public static void polynomialRoots (ArrayList < String > questions,
