...
public class sequences
{
	...
	{				// reciprocals of triangle numbers, general arithmetic, geometric, even, odd,
		...
		{				// general arithmetic
			...
			question =
				("(" + i + ") " + beginning + " + " + (interval + beginning) +
				 " + " + (interval * 2 + beginning) + " + \\ldots " + finalnum +
				 " = ");
			answer =
				String.valueOf (((finalnum + beginning) / 2) *
						((finalnum - beginning) / interval + 1));
		}
		if (choose == 5)
		{				// geometric
			...
			question =
				("(" + i + ") " + beginning + " + " +
				 simplify.getFraction ((num * beginning) + "/" + denom) + " + " +
				 simplify.getFraction ((int) (Math.pow (num, 2) * beginning) + "/" +
					 (int) Math.pow (denom,
						 2)) + " + \\ldots = ");
			answer =
				fracOperations.fracMult (beginning + "/1",
						denom + "/" + (denom - num));
		}
		if (choose == 6)
		{				// first m squares
			int length = rand.nextInt (3) + 5;
			question =
				("(" + i + ") 1 + 4 + 9 + \\ldots " + (int) Math.pow (length, 2) +
				 " = ");
			answer =
				String.valueOf ((length * (length + 1) * (2 * length + 1)) / 6);
		}
		if (choose == 7)
		{				// first m cubes
			int length = rand.nextInt (5) + 5;
			question =
				("(" + i + ") 1 + 8 + 27 + \\ldots " + (int) Math.pow (length, 3) +
				 " = ");
			answer =
				String.valueOf ((int) Math.pow ((length * (length + 1)) / 2, 2));
		}
		if (choose == 8)
		{				// first m squares
			int length = rand.nextInt (3) + 5;
		...
		{				// triangle reciprocals
			...
			question =
				("$" +
				 simplify.getFraction ("1/" + (initial * (initial + 1) / 2), true));
			for (int ii = initial + 1; ii < initial + length; ii++)
			{
				question =
					question + " + \\frac{1}{" +
					String.valueOf (ii * (ii + 1) / 2) + "}";
			}
			question = "(" + i + " ) " + question + " = $";
			answer =
				(fracOperations.
				 fracSub ("2/" + initial, "2/" + (initial + length)));
		}
	}
