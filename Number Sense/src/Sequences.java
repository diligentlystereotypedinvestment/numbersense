...
public class Sequences {
	...
	public void questionGen(int i) { // reciprocals of triangle numbers, general arithmetic, geometric, even, odd,
		...
		if (choose == 5) { // geometric
			...
			question = ("(" + i + ") $" + beginning + " + " + Simp.getFrac((num * beginning) + "/" + denom, false)+ " + "+ Simp.getFrac((int) (Math.pow(num, 2) * beginning) + "/" + (int) Math.pow(denom, 2), false)+ " + \\ldots $= ");
		...
		if (choose == 9) { // triangle reciprocals
			...
			question = ("$" + Simp.getFrac("1/" + (initial * (initial + 1) / 2), false));
	...
	public static void main(String[] args) {
		...
		seq.questionGen(1);
		System.out.println(seq.getQuestion());
		System.out.println(seq.getAnswer());
	}
