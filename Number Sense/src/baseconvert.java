	...
	public static int ognum (String deci, int base)
	{
		...
		if (base < 10)
		{
			for (int i = 1; i < 10; i++)
			{
				...
				tempog =
					(int) ((Integer.valueOf (deci) % 10) * Math.pow (base, i - 1) +
							tempog);
				deci =
					String.
					valueOf ((int)
							(Integer.valueOf (deci) -
							 (Integer.valueOf (deci) % (10))) / 10);
			}
		}
		else
		{
			...
			for (int j = 0; j < eachdeci.length; j++)
			{
				...
				for (int e = 0; e < 26; e++)
				{
					if (eachdeci[eachdeci.length - j - 1].
							equals (String.valueOf (letter)))
					{
						tempog =
							(int) (tempog +
									(int) (letter - 55) * Math.pow (base, j));
					}
					letter++;
				}
				int counter = 0;
				for (int k = 65; k < 26; k++)
				{
					...
					if (!eachdeci[eachdeci.length - j - 1].
							equals (String.valueOf (tempLetter)))
					{
						counter++;
					}
				}
				if (counter == 26)
				{
					tempog =
						(int) (tempog +
								Integer.valueOf (eachdeci[eachdeci.length - j - 1]) *
								Math.pow (base, j));
				}
