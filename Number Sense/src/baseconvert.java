	...
	public static int ognum(String deci, int base) {
		...
		if (base < 10) {
			...
			for (int j = 0; j < eachdeci.length; j++) {
				...
				// }
				// if (!eachdeci[eachdeci.length - j - 1].equals("A") &&
				// !eachdeci[eachdeci.length - j - 1].equals("B") && !eachdeci[eachdeci.length -
				// j - 1].equals("C") && !eachdeci[eachdeci.length - j - 1].equals("D")) {
				// tempog = (int) (tempog + Integer.valueOf(eachdeci[eachdeci.length - j - 1]) *
				// Math.pow(base, j));
				// }
