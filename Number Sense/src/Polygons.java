...
public class Polygons {
	...
	public void gen(ArrayList<String> questions, ArrayList<String> answers, int i) {
		...
		if (sides == 1) { // rectangle
			if (problem == 0) { // diagonals
				questions.add("(" + i + ") What is the diagonal length of a square with an area of "+ (int) Math.pow(sideLength, 2) + "?");
				answers.add("$" + sideLength + "\\sqrt{2}$");
			}
			else if (problem == 1) { // perimeter
				questions.add("(" + i + ") What is the perimeter of a rectangle with a diagonal of " + sideLength+ "$\\sqrt{2}$?");
				answers.add("$" + (4 * sideLength) + "$");
			}
			else if (problem == 2) { // area ratios
				int side2 = 0;
				int side3 = 0;
				int side4 = 0;
				do {
				...
				questions.add("(" + i + ") Rectangle ABCD is " + sideLength + "\" by " + side2+ "\" and rectangle A\'B\'C\'D\' is " + side3 + "\" by" + side4+ "\" What is the ratio of the area of A to B?");
				answers.add(String.valueOf(Simp.getFrac((sideLength * side2) + "/" + (side3 * side4))));
			}
		}
		...
		if (sides == 2) { // circles
			...
			else if (problem == 1) { // circumference
				questions.add("(" + i + ") What is the circumference of a circle with a radius of " + sideLength+ "$\\sqrt{2}$?");
	...
	public void gen2(ArrayList<String> questions, ArrayList<String> answers, int i) {
		...
		if (sides == 2) { // pentagons
			...
			if (problem == 0) { // area
				questions.add("(" + i + ") What is the area of a " + penta.getName() + " with a side length of "+ sideLength + " and an apothem of " + penta.getApothem() + "?");
				answers.add(String.valueOf(penta.getArea()));
			}
			else if (problem == 1) { // perimeter
				questions.add("(" + i + ") What is the perimeter of a " + penta.getName() + "with a side length of "+ sideLength);
				answers.add(String.valueOf(penta.getPerimeter()));
			}
			else if (problem == 2) { // interior angle measures
