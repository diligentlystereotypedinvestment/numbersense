			...
			ArrayList < String > answers, int i)
	{
		...
		{				// addition of two integers
			...
			{			// addition of two mixed numbers
				...
				String printmess =
					("$ (" + i + ") " + addmix.addmix + "\\frac{" +
					 addmix.addfracn + "}{" + addmix.addfracd + "} +" +
					 addmix.addmix2 + "\\frac{" + addmix.addfracn2 + "}{" +
					 addmix.addfracd2 + "}=$");
				questions.add (printmess);	// System.out.print(printmess);
				/*
				 * for (int q = 0; q < 60 - printmess.length(); q++) {
			...
			{			// subtraction of two mixed numbers
				...
				String printmess =
					("$ (" + i + ") " + submix.addmix + "\\frac{" +
					 submix.addfracn + "}{" + submix.addfracd + "} -" +
					 submix.addmix2 + "\\frac{" + submix.addfracn2 + "}{" +
					 submix.addfracd2 + "}=$");
				questions.add (printmess);	// System.out.print(printmess);
				/*
				 * for (int q = 0; q < 60 - printmess.length(); q++) {
			...
			{			// multiplication of two mixed numbers
				...
				String printmess =
					("$ (" + i + ") " + multmix.addmix / 1000 + "\\frac{" +
					 multmix.addfracn + "}{" + multmix.addfracd + "} \\cdot" +
					 multmix.addmix2 / 1000 + "\\frac{" + multmix.addfracn2 + "}{" +
					 multmix.addfracd2 + "}=$");
				questions.add (printmess);	// System.out.print(printmess);
				/*
				 * for (int q = 0; q < 60 - printmess.length(); q++) {
			...
			{			// multiplication of two decimals
				...
				String printmess =
					("$ (" + i + ") " + ((int) (multdeci.adddeci / 100) / 100.0) +
					 "\\cdot" + ((int) (multdeci.adddeci2 / 100) / 100.0) + "=$");
				questions.add (printmess);	// System.out.print(printmess);
				/*
				 * for (int q = 0; q < 60 - printmess.length(); q++) {
				 * //System.out.print("\\textunderscore"); }
			...
			if (rand1_1 == 14)
			{			// division of two mixed numbers
				Addmix divmix = new Addmix ();
				answers.add (String.valueOf (divmix.qmix));
				String printmess =
					("$ (" + i + ") " + divmix.addmix / 1000 + "\\frac{" +
					 divmix.addfracn + "}{" + divmix.addfracd + "} \\div" +
					 divmix.addmix2 / 1000 + "\\frac{" + divmix.addfracn2 + "}{" +
					 divmix.addfracd2 + "}=$");
				questions.add (printmess);	// System.out.print(printmess);
				/*
				 * for (int q = 0; q < 60 - printmess.length(); q++) {
			...
			{			// division of two decimals
				...
				String printmess =
					("$ (" + i + ") " + ((int) divdeci.adddeci / 10.0) + "\\div" +
		...
		{				// comparison
			...
			{
				printmess =
					("(" + i + ") Which is greater, $\\frac{" + compare.addfracn +
					 "}{" + compare.addfracd + "} \\text{ or } " +
					 (double) ((int) (compare.addmix2 / 100)) / 100 + "$");
				if ((1.0 * compare.addfracn / compare.addfracd) >
						(double) ((int) (compare.addmix2 / 100)) / 100)
				{
					answers.add (String.
							valueOf ((1.0 * compare.addfracn /
									compare.addfracd)));
				}
				else
				{
					answers.add (String.
							valueOf ((double) ((int) (compare.addmix2 / 100))
								/ 100));
				}
			}
			else
			{
				printmess =
					("(" + i + ") Which is greater, $\\frac{" + compare.addfracn +
					 "}{" + compare.addfracd + "} \\text{ or } \\frac{" +
					 compare.addfracn2 + "}{" + compare.addfracd2 + "}" + "$");
				if ((1.0 * compare.addfracn / compare.addfracd) >
						(1.0 * compare.addfracn2 / compare.addfracd2))
				{
					answers.add (String.
							valueOf ((1.0 * compare.addfracn /
									compare.addfracd)));
		...
		{				// gcf and lcm
			...
			{			// gcf of two
				printmess =
					("(" + i + ") The GCF of " + num1 + " and " + num2 + " is");
				answers.add (String.valueOf (1.0 * LCMandGCF.GCF (num1, num2)));
			}
			if (choose == 2)
			{			// lcm of two
				printmess =
					("(" + i + ") The LCM of " + num1 + " and " + num2 + " is");
				answers.add (String.valueOf (1.0 * LCMandGCF.LCM (num1, num2)));
			}
			if (choose == 3)
			{			// gcf of three
				printmess =
					("(" + i + ") The GCF of " + num1 + ", " + num2 + ", and " +
					 num3 + " is");
				answers.add (String.
						valueOf (1.0 * LCMandGCF.GCF3 (num1, num2, num3)));
			}
			if (choose == 4)
			{			// lcm of three
				printmess =
					("(" + i + ") The LCM of " + num1 + ", " + (num1 * 1.5) +
					 ", and " + (num1 * 2) + " is");
		...
		{				// percent problems
			...
			String printmess =
				("(" + i + ") P is " + percent + "\\% of Q. Q is " + +percent2 +
		...
		{				// mean, mode, and median problems
			...
			{			// mean
				...
				{
					charseries =
						(charseries + String.valueOf (integer.get (p)) + " , ");
				}
			...
			{			// median
				...
				if (integer.size () % 2 == 0)
					median =
						(((double) integer.get (integer.size () / 2) +
		...
		{				// Remainder Problems
			...
			if (dividend <= 10)
			{
				...
				printmess =
					("(" + i + ") What is the remainder of " + number +
					 " $\\div$ " + dividend + "?");
			...
			{			// expression
				...
				printmess =
					("(" + i + ") What is the remainder of " +
					 remainderex.remainder () + " $\\div$ " + dividend + "?");
		...
		{				// Consumer Type Problems
			...
			if (choose == 1)
			{
				...
				pounds =
					.5 * (rand.nextInt ((int) (Math.log (oz) / Math.log (2))) + 1);
			...
			{
				printmess =
					"(" + i + ") If " + oz + " oz of a liquid costs \\$" +
					eliminateZero.money (price) + " then how much does " + pounds +
					" pounds cost?";
			...
			{
				printmess =
					"(" + i + ") If " + pounds + " pounds of a liquid costs \\$" +
					eliminateZero.money (postPrice) + " then how much does " + oz +
					" oz cost?";
		...
		if (random == 14)
		{				// Number Theory Problems Involving: Prime Numbers, Divisors, Sums of Divisors,
			// etc.
			...
			{			// number of prime divisors
				printmess =
					("(" + i + ") How many prime divisors in " + base + "?");
				answers.add (String.valueOf (primes.prime (base).size ()));
			}
			if (choose == 2)
			{			// sum of prime divisors
				printmess =
					("(" + i + ") What is the sum of the prime divisors " + base +
					 "?");
				answers.add (String.valueOf (primes.primesum (base)));
			}
			if (choose == 3)
			{			// number of composite integers
				printmess =
					("(" + i + ") How many divisors does " + base + " have?");
				answers.add (String.valueOf (composite.num (base)));
			}
			if (choose == 4)
			{			// sum of composite integers
				printmess =
					("(" + i + ") What is the sum of the composite integers in " +
					 base + "?");
				answers.add (String.valueOf (composite.sum (base)));
			}
			if (choose == 5)
			{			// number of prime divisors under n
				printmess =
					("(" + i + ") How many prime divisors under " + base +
					 " does " + base + " have?");
				answers.add (String.valueOf (relative.count (base)));
			}
			if (choose == 6)
			{			// sum of prime divisors under n
				printmess =
					("(" + i + ") What is the sum of the composite integers in " +
					 base + "?");
