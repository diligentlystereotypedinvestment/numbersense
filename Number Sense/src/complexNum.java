...
public class complexNum
{
	...
	public static complexNum multiply (complexNum multiplicand1,
			complexNum multiplicand2)
	{
		complexNum product =
			new complexNum (String.
					valueOf (Integer.valueOf (multiplicand1.getReal ()) *
						Integer.valueOf (multiplicand2.getReal ()) -
						Integer.valueOf (multiplicand1.
							getImaginary ()) *
						Integer.valueOf (multiplicand2.
							getImaginary ())),
					String.valueOf (Integer.
						valueOf (multiplicand1.getReal ()) *
						Integer.valueOf (multiplicand2.
							getImaginary ()) +
						Integer.valueOf (multiplicand1.
							getImaginary ()) *
	...
	public static complexNum divide (complexNum divisor, complexNum dividend)
	{
		...
		int denom =
			...
			valueOf (multiply
					(dividend,
					 new complexNum (dividend.getReal (),
						 String.valueOf (-1 *
		...
		complexNum quotient =
			new complexNum (simplify.getFraction (product.getReal () + "/" + denom),
					simplify.getFraction (product.getReal () + "/" +
						denom));
		return quotient;
	}
			...
			ArrayList < String > answers, int i)
	{
		...
		complexNum multipl1 =
			new complexNum (String.valueOf (rand.nextInt (11) - 5),
		...
		if (solveFor == 0)
		{				//a
			questions.add ("(" + i + ") " + question + "$a$?");
			answers.add (String.valueOf (product.getReal ()));
		}
		if (solveFor == 1)
		{				//b
			questions.add ("(" + i + ") " + question + "$b$?");
			answers.add (String.valueOf (product.getImaginary ()));
		}
		if (solveFor == 2)
		{				//a + b
			questions.add ("(" + i + ") " + question + "$a + b$?");
			answers.add (String.
					valueOf (Integer.valueOf (product.getReal ()) +
						Integer.valueOf (product.getImaginary ())));
		}
		if (solveFor == 3)
		{				//a - b
			questions.add ("(" + i + ") " + question + "$a - b$?");
		...
		{				//a + b
			questions.add ("(" + i + ") " + question + "$a + b$?");
			answers.add (fracOperations.
					fracAdd (quotient.getReal () + "/" + 1,
						quotient.getImaginary ()));
		}
		if (solveFor == 7)
		{				//a - b
			questions.add ("(" + i + ") " + question + "$a - b$?");
			answers.add (fracOperations.
					fracSub (quotient.getReal () + "/" + 1,
						quotient.getImaginary ()));
		}
	}
